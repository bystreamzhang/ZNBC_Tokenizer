"""只依赖 Python 标准库的本地可视化 HTTP server。"""
...
from .adapters.bpe_decode import build_bpe_decode
from .adapters.bpe_encode import build_bpe_encode
from .adapters.bpe_trace import (
...
class VisualizationRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        api_builders = {
            "/api/bpe/trace": build_bpe_trace,
            "/api/bpe/encode": build_bpe_encode,
            "/api/bpe/decode": build_bpe_decode,
        }
        builder = api_builders.get(request_path)
        if builder is None:
        ...
        try:
            ...
            try:
                ...
                raise VisualizationInputError("请求不是有效的 UTF-8 JSON。") from error
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="启动 ZNBC Tokenizer 本地可视化界面。"
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
