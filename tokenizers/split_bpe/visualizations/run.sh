...
server 只监听 127.0.0.1，默认端口是 8010。
按 Ctrl+C 停止。
EOF
}
...
if [[ ! -f "${visualizations_dir}/static/split.html" ]]; then
    echo "错误：没有找到 visualizations/static/split.html。" >&2
    exit 1
fi

cd "${workspace_root}"
exec python3 -m tokenizers.split_bpe.visualizations.server \
    --host 127.0.0.1 \
    --port "${port}"
