"""只依赖 Python 标准库的自研 GPT-4 tokenizer 本地 server。"""
...
from .adapters.own_gpt4_overview import (
...
class VisualizationRequestHandler(BaseHTTPRequestHandler):
    """提供静态页面，并把 JSON 请求交给真实 Python tokenizer。"""
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if request_path != "/api/own-gpt4/overview":
        ...
        try:
            ...
            result = build_own_gpt4_overview(payload)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="启动自研 GPT-4 tokenizer 本地可视化界面。"
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
