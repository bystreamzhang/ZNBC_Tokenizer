...
repo_root="$(cd -- "${visualizations_dir}/.." && pwd)"
...
if [[ ! -f "${repo_root}/bpe.py" ]]; then
    echo "错误：没有找到 ${repo_root}/bpe.py。" >&2
    exit 1
fi
...
cd "${repo_root}"
exec python3 -m visualizations.server --host 127.0.0.1 --port "${port}"
