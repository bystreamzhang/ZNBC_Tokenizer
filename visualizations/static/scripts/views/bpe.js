...
export function initializeBpeView() {
  const elements = {
    ...
    encodingSummary: requiredElement("#encoding-summary"),
    encodingTokens: requiredElement("#encoding-tokens"),
  ...
  function renderEncoding() {
    ...
    elements.encodingSummary.textContent = `${encoding.utf8_byte_count} bytes → ${encoding.token_count} tokens · ${formatRatio(encoding.compression_ratio)} · 减少 ${formatPercentage(encoding.reduction_ratio)}`;
    renderSequences(
      elements.encodingTokens,
      [encoding.tokens],
      result.vocabulary,
      { labels: [encoding.text ? `“${encoding.text}”` : "空 string"] },
    );
  }
  ...
  function renderInvariants() {
    elements.invariants.replaceChildren(
      makeInvariant(
        "逐轮 trace 与 BytePairEncoder.encode() 一致",
        result.invariants.trace_matches_encoder,
      ),
      makeInvariant(
        "编码后的 token bytes 可还原输入",
  ...
  async function run() {
    ...
    elements.status.textContent = "正在运行当前 bpe.py…";
    try {
      result = await requestBpeTrace(payloadFromInputs());
      stepIndex = 0;
      renderAll();
